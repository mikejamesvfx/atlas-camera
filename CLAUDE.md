...
atlas_camera.comfy      � ComfyUI node library (91 nodes + 6 experimental + 3 legacy, no hard Comfy dep;
...
The full node catalog (91 nodes + 6 experimental + 3 legacy), `comfy/` module layout,
...
- Registered node keys + display names are a saved-workflow contract
  (`tests/test_comfy_node_registry.py` pins the surface; currently 91 + 6 + 3 legacy).
