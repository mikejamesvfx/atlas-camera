...
atlas_camera.comfy      � ComfyUI node library (92 nodes + 6 experimental + 2 legacy, no hard Comfy dep;
...
The full node catalog (92 nodes + 6 experimental + 2 legacy), `comfy/` module layout,
...
- Registered node keys + display names are a saved-workflow contract
  (`tests/test_comfy_node_registry.py` pins the surface; currently 92 + 6 + 2 legacy).
...
- Derive nodes CLOBBER prior PROXY_ROLE geometry (deliberate);
  hand-authored `AtlasAddPlanePolygon` planes APPEND (chainable);
