...
atlas_camera.comfy      � ComfyUI node library (82 nodes + 4 experimental, no hard Comfy dep;
...
The full node catalog (82 nodes + 4 experimental), `comfy/` module layout,
...
- Registered node keys + display names are a saved-workflow contract
  (`tests/test_comfy_node_registry.py` pins the surface; currently 82 + 4).
