...
atlas_camera.comfy      � ComfyUI node library (85 nodes + 5 experimental + 2 legacy, no hard Comfy dep;
...
The full node catalog (85 nodes + 5 experimental + 2 legacy), `comfy/` module layout,
...
- Registered node keys + display names are a saved-workflow contract
  (`tests/test_comfy_node_registry.py` pins the surface; currently 85 + 5 + 2 legacy).
