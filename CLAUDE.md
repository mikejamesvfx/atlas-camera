...
atlas_camera.comfy      � ComfyUI node library (92 standard + 6 experimental + 2 legacy + 2 iOS, no hard Comfy dep;
...
The full node catalog (92 standard + 6 experimental + 2 legacy + 2 iOS = 102
...
- Registered node keys + display names are a saved-workflow contract
  (`tests/test_comfy_node_registry.py` pins the surface; currently 92 standard
  + 6 experimental + 2 legacy + 2 iOS).
...
- Derive nodes CLOBBER prior PROXY_ROLE geometry (deliberate);
  viewport-drawn (Draw -> Apply) planes APPEND, never clobber;
