...
| `AtlasAddPatchView` | ATLAS_SOLVE, patch_image (IMAGE), ±azimuth_deg, ±elevation_deg, ±distance_scale, ±name, ±depth_model, ±relief_grid, ±priority, ±device | ATLAS_SOLVE | Adds an AI novel-view "patch" to fill areas the primary camera can't see (occluded/grazing). Takes a novel view generated at a defined angle (e.g. Qwen-Image-Edit-2511 + Multiple-Angles LoRA at ~25-45° left/right/above), constructs a patch camera by orbiting the recovered camera around the scene pivot by that same angle (`camera_math.orbit_camera` — shares the primary's world frame), derives the patch view's own relief geometry (Depth Anything) in that frame, and appends a `ProjectionSource` to the solve. Chain one per angle; the viewport layers them over the primary with a facing-ratio mask. **The angle set here MUST match the angle asked of the LoRA** — the patch camera is placed at exactly this nominal orbit. See the "Multi-angle patch projection" design rule. Needs `[neural]` |
