...
- **`scene_type` convenience preset (still artist-selected, not auto-detected):** `AtlasDeriveProjectionGeometry`'s `scene_type` widget (`manual`/`organic`/`indoor`/`outdoor`, default `manual`) is a one-choice shortcut over the three parameters above, resolved in a class-level `_SCENE_TYPE_PRESETS` dict at the top of `derive()` — purely a convenience mapping onto the exact same code paths, never a new solving strategy: `organic` → `geometry_mode="relief_mesh"` (matches the "organic/natural environments" framing used when the RANSAC/room-cuboid methods were first proposed as alternatives to plain relief-mesh); `indoor` → `primitives`+`room_cuboid`+the Indoor Depth-Anything-V2 model; `outdoor` → `primitives`+`ransac_planes`+the Outdoor model. `manual` (default) leaves `geometry_mode`/`primitive_method`/`depth_model` exactly as their own widgets say — fully backward-compatible with saved workflows predating this widget. The preset only reaches this node's own depth estimation; if `AtlasLearnedSolveFromImage` upstream uses `height_mode=measure_from_depth`, its separate `depth_model` widget must be set to match by hand (nodes don't cross-configure each other's widgets). Recorded in `debug_metadata["proxy_derivation"]["scene_type"]` for traceability, same pattern as `primitive_method`.
