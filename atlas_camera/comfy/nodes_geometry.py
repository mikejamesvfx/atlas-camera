...
class AtlasRetopologizeLayer:
    ...
    def retopo(self, solve, layer="", method="decimate", target_vertex_count=2000,
        ...
        retopo_sources = ("depth_relief_mesh", "viewport_polygon", "viewport_box")
...
def _plate_data_url(image_tensor, max_edge=1280):
    """The plate as a base64 PNG data URL for the polygon canvas widget.

    Sent through the node's `ui` payload rather than sniffed off an upstream
    LiteGraph node's preview, so the canvas works regardless of which node
    supplies the image. Downscaled: this is a click surface, not a render.
    """
    pil = _image_tensor_to_pil(image_tensor)
    if max(pil.size) > max_edge:
        ratio = max_edge / float(max(pil.size))
...
class AtlasAddPlanePolygon:
    """✏️ Draw a plane: click an outline on the plate, get a plane clipped to it.

    Every AtlasDerive* node fits planes from depth and emits axis-extent
    RECTANGLES around each cluster — a rectangle covers image area the real
    surface never occupied, and since the viewport projects by WORLD POSITION
    (geometry UVs are not used for projection), that overshoot receives paint
    from unrelated parts of the plate. That is the smear on derived walls.

    Here the artist supplies the outline, so the emitted mesh covers exactly
    the clicked region. Outlines are N-gons: four points for a wall, more for
    an L-shaped facade or a traced roofline.

    Unlike the derive nodes, this one APPENDS — a hand-authored surface is an
    addition, not a re-derivation of the whole set. Chain several of them (and
    hang them off a solve that already carries a relief mesh) with no
    AtlasMergeGeometry in between.

    ``fit_mode``:
    - ``auto`` (default) — plane RANSAC over the depth INSIDE the outline,
      falling back to the rectangle solve when that region's depth is
      untrustworthy. The report always names which one ran.
    - ``depth_ransac`` — force the depth fit; a poor region fails visibly
      instead of falling back.
    - ``rectangle`` — depth-free single-view metrology: treats a quad as a
      real-world rectangle and recovers orientation from the vanishing points
      of its two edge pairs. For glass, water, or flat sky-lit facades where
      monocular depth is useless. Quads only.

    Leave ``depth`` unwired to work rectangle-only (e.g. before any depth
    model has run).
    """
    RETURN_TYPES = ("ATLAS_SOLVE", "STRING")
    RETURN_NAMES = ("solve", "report")
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "depth": ("ATLAS_DEPTH_MAP", {
                    "tooltip": "Shared metric depth. Unwired = rectangle solve only."}),
                "image": ("IMAGE", {
                    "tooltip": "The plate to click on. Only the canvas widget uses it — "
                               "geometry comes from the solve's own intrinsics."}),
                "polygons": ("STRING", {"multiline": True, "default": "",
                    "tooltip": "Outlines as JSON, written by the canvas widget and "
                               "hand-editable. Points are 0..1 plate-relative, so "
                               "changing plate resolution does not invalidate them."}),
                "fit_mode": (["auto", "depth_ransac", "rectangle"], {"default": "auto",
                    "tooltip": "auto = depth RANSAC with a rectangle fallback (reported). "
                               "depth_ransac = fail rather than fall back. rectangle = "
                               "depth-free vanishing-point solve, quads only."}),
                "name_prefix": ("STRING", {"default": "hand_plane"}),
                "min_inlier_fraction": ("FLOAT", {"default": 0.35, "min": 0.0, "max": 1.0,
                    "step": 0.05,
                    "tooltip": "RANSAC consensus below this triggers the rectangle "
                               "fallback in auto mode."}),
            },
        }

    @classmethod
    def IS_CHANGED(cls, polygons="", fit_mode="auto", **_):
        # The outlines are a persisted widget that decides what geometry
        # exists; without a content fingerprint ComfyUI serves a cached solve
        # after an outline is edited.
        import hashlib
        return hashlib.md5(f"{fit_mode}|{polygons}".encode()).hexdigest()

    def add(self, solve, depth=None, image=None, polygons="", fit_mode="auto",
            name_prefix="hand_plane", min_inlier_fraction=0.35):
        ...
        np = _require_numpy()
        out = copy.deepcopy(solve)
        plate = _plate_data_url(image) if image is not None else None

        def _done(report, statuses=()):
            ui = {"atlas_polygon_status": [json.dumps(list(statuses))],
                  "atlas_plate": [plate] if plate else []}
            return {"ui": ui, "result": (out, report)}

        text = (polygons or "").strip()
        if not text:
            return _done("AtlasAddPlanePolygon: no polygons drawn.")
        try:
            blob = json.loads(text)
            records = list(blob["polygons"]) if isinstance(blob, dict) else list(blob)
        except Exception as exc:
            return _done(f"AtlasAddPlanePolygon: polygons could not be read ({exc}).")

        size = {}
        if depth is not None:
            size = dict(width=int(depth.image_width), height=int(depth.image_height))
        elif image is not None:
            size = dict(width=int(image.shape[2]), height=int(image.shape[1]))
        spec = CameraSpec.from_solve(out, **size)
        if not spec.has_focal:
            return _done("AtlasAddPlanePolygon: solve has no focal length; nothing added.")
        width, height, fx, fy, cx, cy = spec.as_params()
        view_matrix = out.camera.extrinsics.camera_view_matrix

        if depth is not None:
            depth_map = _depth_map_for_solve(depth, width, height)
        else:
            # No depth wired: every pixel is unusable, so the rectangle solve
        ...
        for index, record in enumerate(records):
            ...
            poly_id = record.get("id") or f"polygon_{index + 1}"

            def _skip(reason, _label=label, _id=poly_id):
                lines.append('"%s": skipped(%s)' % (_label, reason))
                statuses.append({"id": _id, "ok": False, "note": reason})

            if not record.get("enabled", True):
                _skip("disabled")
                continue

            mode = record.get("fit_mode") or "inherit"
            mode = fit_mode if mode == "inherit" else mode
            try:
                points = points_from_normalized(record.get("points", []), width, height)
            except (ValueError, TypeError) as exc:
                _skip("bad points (%s)" % exc)
                continue
            ...
            out.projection_scene.proxy_geometry.append(AtlasProxyPrimitive(
                ...
                metadata={
                    ...
                    "distance_m": float(fit.distance_m),
                    "fit_stats": fit.stats,
            ...
            detail = " — %s" % fit.reason if fit.reason else ""
            lines.append('%s "%s": ok(%s, confidence %.2f)%s'
                         % (name, label, fit.method, fit.confidence, detail))
            statuses.append({"id": poly_id, "ok": True, "method": fit.method,
                             "confidence": round(float(fit.confidence), 4),
                             "note": fit.reason})

        if made:
            out.projection_scene.debug_metadata["hand_polygons"] = {
