...
class AtlasMegaPipeline:
    ...
    def execute_pipeline(self, image: Any, output_dir: str, camera_height_m: float = 1.6, scene_type: str = "outdoor"):
        ...
        # 2. Derive Geometry. AtlasDeriveProjectionGeometry is the god-node that
        # estimates its OWN depth from image + depth_model — it takes no `depth`
        # input (only the one-job composable nodes, e.g. AtlasDeriveReliefMesh,
        # consume a pre-computed ATLAS_DEPTH_MAP). NOTE: when scene_type is a
        # preset (anything but "manual"), that preset overrides geometry_mode /
        # primitive_method / depth_model; the explicit values below take effect
        # only under scene_type="manual".
        logging.info("  -> Deriving Projection Geometry (depth estimated internally)...")
