...
| `AtlasPlanarHolePatch` | ATLAS_SOLVE, hole_mask (MASK), ±layer, ±ring_cells, ±max_components, ±normal_tolerance_deg, ±max_plane_error_m, ±max_hole_fraction, ±enclosed_only, ±min_normal_support_fraction, ±max_patch_edge_factor, ±max_patch_depth_factor | solve (ATLAS_SOLVE), remaining_holes (MASK), report (STRING), created_islands (MASK) | ◩ Normal-guided planar completion for selected relief holes. Recovers the compacted image-space lattice, groups missing quads, rejects frame/oversize regions before the fit budget, and attempts eligible components smallest-first. Each island averages the largest agreeable boundary-normal cluster and validates its densest plane-position band. Two independent scale-invariant gates run after ray/plane intersection: `max_patch_edge_factor` constrains generated world edges against their source-pixel span × local metres-per-pixel; appended `max_patch_depth_factor` (default 2×) constrains both forward and backward camera-depth excursion against the fitted support plane's local 5–95% depth band. Either gate rejects the complete island atomically, preventing Z needles even when a relaxed edge budget is used. Accepted patches reuse perimeter indices and exact projective UVs, so they are stitched into the relief mesh. `created_islands` is exactly `selected hole_mask - remaining_holes`; use it directly as an inpaint mask or viewport patch layer. Run before `AtlasRetopologizeLayer`. Geometry cannot invent a hidden texture — pair real disocclusions with a clean plate |

| `AtlasPathGuidedHoleRepair` | ATLAS_SOLVE, hole_mask (MASK), camera_path (ATLAS_CAMERA_PATH), ±path_frames (IMAGE), ±paint_mask (MASK), ±exclude_mask (MASK), ±layer, ±frame_offset_from_end, ±lens_scale_override, ±resolution, ±selection_mode, ±max_selected_islands, ±candidate-fit controls | repair_mask (MASK), angle_preview (IMAGE), visible_islands (MASK), report (STRING) | 🎥 Converts a Camera Path repair angle into an exact source-space tear selection for a scoped second `AtlasPlanarHolePatch`. Geometry always samples the complete, tiny parametric `camera_path`; `path_frames` is preview-only and may contain one explicitly indexed snapshot from **Bake Repair Frame**. If `frame_offset_from_end` requests an image that was not baked, the node uses a black background and reports the mismatch instead of showing a misregistered frame. `exclude_mask` is subtracted before component discovery, separating open foreground tears from the exterior background/sky island. Candidate planes receive stable IDs and are depth-rasterized at the selected path pose; visible or painted IDs map back to complete source-space components. `all_visible` is unattended mode; `paint_overlap` consumes a moved-angle mask. Feed `repair_mask` into a normal Planar Hole Patch whose edge and depth gates remain final acceptance. Use after a conservative first pass and run retopology after Pass 2 |
...
- `examples/atlas_path_guided_hole_repair_workflow.json` — the agent-ready two-pass repair graph. A conservative first Planar Hole Patch feeds an authoring viewport. Choose Orbit/Arc, widen the playback lens if useful, and click **Bake Repair Frame**: only the indexed final JPEG is embedded, while the complete parametric camera path remains available. Use **Bake Full Path** only for video. `AtlasPathGuidedHoleRepair` subtracts the upstream background/sky exclusion, renders stable candidate-island IDs over the matching frame, and returns exact source-space components. Pass 2 uses a measured 40× edge cap plus the independent 2× local-depth excursion gate. On the 8K engine workflow this retains 18 safe open-edge repairs (accepted maximum 27.8×) while atomically rejecting the exact 135-cell, 46-vertex/270-face island that produced the forward/backward needles at 45.3×. Retopology then receives only accepted repair geometry |
