...
### Node catalog (79 nodes + 4 experimental)
...
| `AtlasStereoRender` | solve (ATLAS_SOLVE), source_image (IMAGE), ±interocular_m (0.065), ±convergence_m (0 = parallel), ±output_mode (`sbs`/`sbs_half`/`anaglyph`/`separate`), ±resolution (1024) | stereo, left, right (IMAGE), report (STRING) | 👓 Geometry-true stereo pair rendered HEADLESSLY from the layered projection scene (`core/projection_render.py` — the promoted `headless_evidence` rasterizer, z-buffered, perspective-correct UVs; each layer samples its own projector texture, so the matte-painting property survives the eye offset). Eyes translate ±io/2 along camera-right from the 4×4 view matrix; convergence is SHIFTED-SENSOR (`cx` offset), never toe-in. Unlike depth-warp stereo nodes there are no hole-filling heuristics: disocclusions beyond the layered coverage render as holes (fill with clean-plate bands). Interocular is metric — check the 🩺 scale verdict first. Pure-numpy rasterizer: keep `resolution` moderate on dense relief meshes |
...
| `AtlasPlanarHolePatch` | ATLAS_SOLVE, hole_mask (MASK), ±layer, ±ring_cells, ±max_components, ±normal_tolerance_deg, ±max_plane_error_m, ±max_hole_fraction, ±enclosed_only, ±min_normal_support_fraction, ±max_patch_edge_factor | solve (ATLAS_SOLVE), remaining_holes (MASK), report (STRING), created_islands (MASK) | ◩ Normal-guided planar completion for selected relief holes. Recovers the compacted image-space lattice, groups missing quads, rejects frame/oversize regions before the fit budget, then uses a bounded heap to select and attempt eligible components smallest-first (`O(n log k)` for `k=max_components`). Each island finds the largest surrounding normal cluster inside `normal_tolerance_deg`; it fills when that cluster reaches `min_normal_support_fraction` (default 30%), using the average of the agreeing normals. `max_plane_error_m` selects and validates the densest local plane-position band when the same angle exists at multiple depths. After ray/plane intersection, `max_patch_edge_factor` (default 20×) rejects spikes using a dimensionless per-island stretch: generated world-edge length divided by its source-pixel span × the boundary's median metres-per-pixel. This explicitly groups image resolution, recovered camera/world scale, perspective and relief-grid spacing; the same setting is invariant to an 8K/1K resize or a uniform metric scene rescale. The report names the exact failed gate and measured agreement/residual/stretch. Accepted patches reuse perimeter indices and exact projective UVs, so they are stitched into the relief mesh rather than emitted as wall/box primitives. `created_islands` is exactly `selected hole_mask - remaining_holes` in source-image space: preview it directly, use it as an inpaint region, or wire it to `AtlasBlockoutViewport.patch_mask` for magenta layer identity and a moved-camera mask render. Defaults reject frame/sky openings and components over 5% of the grid. Run before `AtlasRetopologizeLayer`; retopo then receives relief + accepted patches as one mesh. Geometry cannot invent a hidden texture — pair real disocclusions with a clean plate |

| `AtlasPathGuidedHoleRepair` | ATLAS_SOLVE, hole_mask (MASK), camera_path (ATLAS_CAMERA_PATH), ±path_frames (IMAGE), ±paint_mask (MASK), ±exclude_mask (MASK), ±layer, ±frame_offset_from_end, ±lens_scale_override, ±resolution, ±selection_mode, ±max_selected_islands, ±candidate-fit controls | repair_mask (MASK), angle_preview (IMAGE), visible_islands (MASK), report (STRING) | 🎥 Converts a Camera Path repair angle into an exact source-space tear selection for a scoped second `AtlasPlanarHolePatch`. `frame_offset_from_end=0` means the final path frame; `lens_scale_override=0` inherits the path playback/bake focal multiplier (below 1.0 widens). `exclude_mask` is subtracted before component discovery, separating foreground tears that open onto the silhouette from the otherwise exterior-sized background/sky island. Missing pixels cannot be ray-cast from the moved view, so the node fits relaxed candidate planes, assigns each connected source island a stable integer ID, depth-rasterizes those candidates against the existing relief at the selected path pose, and maps visible/painted IDs back to their complete original-image cells. `all_visible` is the unattended agent mode; `smallest_visible`/`largest_visible` plus `max_selected_islands` provide bounded batches; `paint_overlap` consumes a mask painted on `angle_preview`. The selector bypasses only the preview edge-stretch gate—its `repair_mask` must feed a normal Planar Hole Patch whose own thresholds remain the geometry acceptance gate. Use after a conservative first pass and connect the same sky/background exclusion used upstream; run retopology after the second pass |
...
| `AtlasDeriveWalls` | ATLAS_SOLVE, depth (ATLAS_DEPTH_MAP), ±max_walls, ±max_objects, ±distance_modes, ±exclude_mask, ±ground_anchor | ATLAS_SOLVE | One job: vertical walls (azimuth_walls), with optional foreground boxes/cylinders. `max_objects` defaults to **0** so a wall-only request cannot silently add cubes; raise it explicitly for aerial/top-down building blockouts. Truncates sloped roofs/spires — use `AtlasDeriveTowersSpires` for those. `distance_modes`/`exclude_mask`: see the skyline design rule |

| `AtlasDeriveTowersSpires` | ATLAS_SOLVE, depth (ATLAS_DEPTH_MAP), ±max_walls, ±max_objects, ±distance_modes, ±exclude_mask, ±ground_anchor, ±roofline_split | ATLAS_SOLVE | One job: walls extruded to the real silhouette top (vertical_extrusion) — reaches towers/spires/sloped roofs `AtlasDeriveWalls` truncates. `max_objects` defaults to **0**; boxes/cylinders are opt-in. `distance_modes`/`exclude_mask`: see the skyline design rule |
...
| `AtlasDepthDetailEnhance` | depth (ATLAS_DEPTH_MAP), ±strength (0.35), ±detail_cutoff_px (64), ±exclude_mask (MASK) | depth (ATLAS_DEPTH_MAP), report (STRING) | 🔬 Embosses the normal map's high-frequency shape onto the shared depth: Frankot-Chellappa integration of `depth.normal` (`core/depth_detail.py`, pure-numpy FFT), high-passed above `detail_cutoff_px` so the metric base can never tilt/re-scale, blended multiplicatively in log-depth and median-renormalized (scale-preserving by construction). COPY-not-mutate like `AtlasMogeNormals`; soft pass-through when the depth carries no normals. Wire `exclude_mask` = sky |
| `AtlasDepthCombine` | depth_base (ATLAS_DEPTH_MAP), depth_detail_src (ATLAS_DEPTH_MAP), ±mode (`high_freq_detail`/`min`/`max`/`masked`), ±strength (0.5), ±detail_cutoff_px (64), ±blend_mask (MASK) | depth (ATLAS_DEPTH_MAP), report (STRING) | ➕ Combines two shared depth maps: `high_freq_detail` grafts the source's fine structure onto the base's metric far-field (the "MoGe detail on V2 exterior" combo, log-domain + median-preserving); `min`/`max` per-pixel envelope; `masked` lerp under `blend_mask`·strength. Returns a COPY carrying the base's `is_metric`; metric/relative mixing is reported as a warning |
| `AtlasPlanarUnwarp` | solve (ATLAS_SOLVE), image (IMAGE), ±plane_name (STRING, "" = ground), ±px_per_meter (0 = auto), ±center_u_m/±center_v_m, ±extent_u_m/±extent_v_m (0 = auto-fit), ±max_resolution (4096) | flat_image (IMAGE), flat_mask (MASK), warp_spec (ATLAS_WARP_SPEC), report (STRING) | ▱ Flattens a solved plane into an editable orthographic texture via the homography the recovered camera implies (`core/planar_projection.py`, built strictly from the 4×4 view matrix). Default = the ground (Y=0); `plane_name` selects a named plane proxy (facade/wall/backdrop) with SOFT fallback to ground + available names in the report. `flat_mask` 1 = a real plate pixel landed (0 = out-of-frame/behind-camera/grazing) — inpaint the zeros, edit the ones. `ATLAS_WARP_SPEC` rides by reference (never serialized) |
| `AtlasPlanarRewarp` | warp_spec (ATLAS_WARP_SPEC), edited_flat (IMAGE), original_image (IMAGE), ±edit_mask (MASK), ±feather_px (4) | image (IMAGE), coverage_mask (MASK) | ▱ Inverse of `AtlasPlanarUnwarp`: warps the edited flat back and composites it into the plate, feathered over `feather_px`; only pixels mapping into the flat rect (∩ `edit_mask` when given, ∩ the unwarp's real-pixel alpha) are touched — off-plane pixels pass through bit-exact. Perspective-correct matte-paint round trip |
...
| `AtlasDeband` | image (IMAGE), ±strength (0.5), ±band_threshold (2.0 LSB), ±radius_px (24), ±preserve_detail (0.5), ±add_grain (0) | image (IMAGE), report (STRING) | 🎚 Model-free debanding for 8-bit-born plates (AI images/JPEGs) before projection/sky dome (`plate/deband.py`). Gradient-gated: only quantization plateaus (luma gradient < `band_threshold`/255) are smoothed via a masked normalized blur; real edges gate to zero and pass bit-exact; the correction is range-clamped to a few banding steps so nothing bleeds across an edge. Float-safe (never quantizes/clamps HDR). Deliberately IMAGE-only — structurally cannot touch an `ATLAS_PLATE_REF`, so a debanded tensor can never masquerade as the registered final plate (P0 trust tier) |
...
- `examples/atlas_planar_hole_patch_workflow.json` — the normal-guided relief repair path: LoadImage → learned solve/shared exterior depth → DeriveReliefMesh → PlanarHolePatch → RetopologizeLayer → Viewport. The viewport is wired directly from retopology so optional wall proxies cannot hide the repair; the disconnected DeriveWalls/AtlasMergeGeometry branch remains available for deliberate experiments. `remaining_holes` feeds both the viewport debug matte and a standalone preview, while `created_islands` feeds its own preview plus the viewport `patch_mask`. 🎨 Layers marks those generated regions magenta; after any camera move, **Render Proxy Passes** fills the appended `patch_render_mask` output and its preview for inpaint. The node report feeds a visible text terminal. The live-tuned defaults use the closest two support rings, `max_components=512`, 20% one-sided normal support, a 0.30 m plane-position cap, and the 20× source-pixel/local-metric edge stretch gate. Each accepted island uses the average of its agreeing normals plus the densest local position band. The bundled `example.png` keeps the graph portable; choose the shot plate in LoadImage. Three notes pin the rejection controls, clean-plate limitation, and the fact that MergeGeometry appends wall primitives rather than boolean-welding them.

- `examples/atlas_path_guided_hole_repair_workflow.json` — the agent-ready two-pass repair graph. A conservative first Planar Hole Patch feeds an authoring viewport. Choose Orbit/Arc, widen the playback lens if useful, and Bake Proxy Path. `AtlasPathGuidedHoleRepair` selects `last frame − frame_offset_from_end`, subtracts an optional upstream background/sky `exclude_mask`, renders stable candidate-island IDs over that baked frame, and outputs the exact source cells of every visible (or artist-painted) foreground island. A scoped second Planar Hole Patch uses the higher 65× edge budget only on that mask, then retopology and a separate final viewport show the result. The report exposes selected frame, inherited/overridden lens, excluded pixel count, candidate/visible/selected counts, and fit rejections for MCP iteration. The 65× example preset was calibrated on the 7680×4512 `atlas_00025.png` scene: it accepted the 59.3× tear while preserving the 154.5× outlier as a spike-safe rejection |
