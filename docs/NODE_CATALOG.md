...
- `examples/atlas_planar_hole_patch_workflow.json` — the normal-guided relief repair path: LoadImage → learned solve/shared exterior depth → DeriveReliefMesh → PlanarHolePatch → RetopologizeLayer → Viewport. The viewport is wired directly from retopology so optional wall proxies cannot hide the repair; the disconnected DeriveWalls/AtlasMergeGeometry branch remains available for deliberate experiments. `remaining_holes` feeds both the viewport debug matte and a standalone preview, while the node report feeds a visible text terminal. The live-tuned defaults use the closest two support rings, `max_components=512`, 20% one-sided normal support, and a 0.30 m plane-position cap; on `flux_00645_.jpg` they filled 295/296 eligible enclosed islands while retaining one measured non-planar rejection and all frame/sky safety rejections. Each accepted island uses the average of its agreeing normals plus the densest local position band. The bundled `example.png` keeps the graph portable; choose the shot plate in LoadImage. Three notes pin the rejection controls, clean-plate limitation, and the fact that MergeGeometry appends wall primitives rather than boolean-welding them.
