...
| `AtlasPathGuidedHoleRepair` | ATLAS_SOLVE, hole_mask (MASK), camera_path (ATLAS_CAMERA_PATH), ±path_frames (IMAGE), ±paint_mask (MASK), ±layer, ±frame_offset_from_end, ±lens_scale_override, ±resolution, ±selection_mode, ±max_selected_islands, ±candidate-fit controls | repair_mask (MASK), angle_preview (IMAGE), visible_islands (MASK), report (STRING) | 🎥 Converts a Camera Path repair angle into an exact source-space tear selection for a scoped second `AtlasPlanarHolePatch`. `frame_offset_from_end=0` means the final path frame; `lens_scale_override=0` inherits the path playback/bake focal multiplier (below 1.0 widens). Missing pixels cannot be ray-cast from the moved view, so the node fits relaxed candidate planes, assigns each connected source island a stable integer ID, depth-rasterizes those candidates against the existing relief at the selected path pose, and maps visible/painted IDs back to their complete original-image cells. `all_visible` is the unattended agent mode; `smallest_visible`/`largest_visible` plus `max_selected_islands` provide bounded batches; `paint_overlap` consumes a mask painted on `angle_preview`. The selector bypasses only the preview edge-stretch gate—its `repair_mask` must feed a normal Planar Hole Patch whose own thresholds remain the geometry acceptance gate. Use after a conservative first pass and after excluding sky/background; run retopology after the second pass |
...
- `examples/atlas_path_guided_hole_repair_workflow.json` — the agent-ready two-pass repair graph. A conservative first Planar Hole Patch feeds an authoring viewport. Choose Orbit/Arc, widen the playback lens if useful, and Bake Proxy Path. `AtlasPathGuidedHoleRepair` selects `last frame − frame_offset_from_end`, renders stable candidate-island IDs over that baked frame, and outputs the exact source cells of every visible (or artist-painted) island. A scoped second Planar Hole Patch uses the higher 65× edge budget only on that mask, then retopology and a separate final viewport show the result. The report exposes selected frame, inherited/overridden lens, candidate/visible/selected counts, and fit rejections for MCP iteration. The 65× example preset was calibrated on the 7680×4512 `atlas_00025.png` scene: it accepted the 59.3× tear while preserving the 154.5× outlier as a spike-safe rejection |
