...
| `AtlasOcclusionGraph` 🕸 | ATLAS_SOLVE, ±depth (ATLAS_DEPTH_MAP) | ATLAS_SOLVE, report (STRING) | Decomposes the solved scene into surface/object/ground/backdrop nodes plus one `occludes` edge per silhouette tear (nearer side occludes), and assigns each node a single permitted `completion_policy`. Attached to the reserved `LatentScene.semantics` slot, so it rides the exported solve JSON. Analysis only — never touches geometry. Confidence comes from `core.scene_health` alone. **A tear it cannot classify licenses NOTHING** (`policy=none`), and one unclassifiable tear blocks completion of the whole surface it exposes — building over the rest would hide the unknown behind geometry that looks finished. Needs `AtlasDeriveProjectionGeometry` upstream for fitted surfaces; without depth it lists the parts and says it analysed no tears. `completion_policy` values are APPEND-ONLY (they serialize into workflows) |
| `AtlasMoveBudget` 📐 | ATLAS_SOLVE, ±camera_path (ATLAS_CAMERA_PATH), ±threshold, ±bisect_steps, ±max_dolly_m, ±max_angle_deg, ±backend | ATLAS_SOLVE, report (STRING) | How far the camera can move before a tear opens. Rasterizes the scene from candidate cameras with a real z-buffer and measures **sealed-minus-covered** — pixels the untorn relief surface covers but real geometry does not. That definition is deliberate: counting uncovered pixels reports zero disocclusion in any scene carrying a backdrop, because the cyclorama covers everything. Reports a per-DOF envelope; connect `camera_path` for a per-frame verdict on the authored move. **Pan/tilt unbounded is correct, not a bug** — rotation about the optical centre produces no parallax. torch rasterizer (cuda→mps→cpu); the numpy path is a reference oracle ~60× slower. Needs a relief mesh; a missing one reports rather than raises |
| `AtlasCompleteDepth` 🩹 | ATLAS_SOLVE, depth (ATLAS_DEPTH_MAP), ±holes (MASK), ±use_diffusion, ±use_normals, ±diffusion_iterations | ATLAS_DEPTH_MAP, hidden_mask (MASK), report (STRING) | Fills depth holes **before** the relief mesh is built, so no tear exists to repair afterwards. Three tiers, best first, each pixel tagged: **ray-plane** (exact intersection of the pixel's own ray with a graph-fitted plane — not an interpolation), **tangent** (first-order continuation from predicted normals), **diffusion** (last resort, evidence-free, labelled). Connect an `AtlasOcclusionGraph` solve for the exact tier and to carry its refusals — a `policy=none` node contributes no plane, so the tear it guards stays open. Measured depth is never overwritten; `hidden_mask` marks every synthesized pixel and the report gives a trust-weighted confidence |
