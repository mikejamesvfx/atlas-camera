...
- **CPU-first, zero-dependency core.** The core is pure Python/NumPy. `torch` is
  imported lazily *inside* the neural nodes, so **every node registers with no
...
- In the ComfyUI node module, `import torch` / `import numpy` live **inside** the
  functions that use them — never at module load — so **every node registers in
...
| GPU to load | Required (CUDA torch + weights) | Usually required | **None — every node registers on CPU; GPU only for AI features** |
