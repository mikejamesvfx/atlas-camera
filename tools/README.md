...
| `ground_consensus_probe.py` | Measures ground recovery on a real RAW plate: cross-model camera-height disagreement (V2 vs MoGe), the shipping estimator instrumented with a residual-vs-range profile, `core.ground_consensus` over the ROI/mask/weighting grid, and independent metric scale from a known-size object. The measurements behind `docs/development/gravity-locked-ground-experiment.md`. Needs `[raw]` + `[neural]` + `[moge]`; set `XFORMERS_DISABLED=1` on a GPU newer than xformers' kernels. |
